...
| **`SLACK_ORDER_WEBHOOK_URL`**         | Tạo tại Slack App → Incoming Webhooks, dán URL webhook của room chung team                                      |
| **`MAIL_USERNAME` / `MAIL_PASSWORD**` | Đăng ký tài khoản free tại [mailtrap.io](https://mailtrap.io) → Lấy SMTP credentials của inbox test            |
...
> Lệnh trên sẽ tạo symbolic link từ `public/storage` → `storage/app/public`, giúp ảnh upload được truy cập công khai qua đường dẫn URL trình duyệt.
