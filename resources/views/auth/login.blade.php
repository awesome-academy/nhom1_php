...
<div class="relative flex min-h-[calc(100vh-5rem)] items-center justify-center overflow-hidden px-4 py-24 sm:px-6 lg:px-8"
    ...
    <div class="pointer-events-none absolute left-1/2 top-[8%] h-[420px] w-[min(560px,92vw)] -translate-x-1/2 rounded-b-[260px] opacity-50 blur-[2px]"
        ...
        <div class="relative rounded-[28px] border border-[#EADBCE] bg-white/95 p-9 shadow-[0_30px_70px_rgba(43,30,25,0.10)] backdrop-blur-sm sm:p-11">
            ...
            <div class="relative text-center">
                ...
                <h1 class="mt-2 font-serif text-[31px] font-semibold leading-tight tracking-tight text-[#2B1E19]">
                    {{ __('Chào mừng trở lại') }}
                </h1>
                <p class="mt-2.5 text-[13.5px] leading-relaxed text-[#736357]">
                    {{ __('Đăng nhập để tiếp tục thưởng thức cà phê và bánh tươi hảo hạng') }}
                </p>
            </div>

            <!-- Session Status Alert -->
            <x-auth-session-status class="relative mt-6" :status="session('status')" />

            <!-- 2. Form Đăng nhập -->
            <form method="POST" action="{{ route('login') }}" class="relative mt-7 space-y-[18px]">
                @csrf
                ...
                <!-- Password Input (Có Alpine.js Toggle Show/Hide Password) -->
                <div x-data="{ show: false }">
                    ...
                    <div class="relative">
                        ...
                        <button
                            type="button"
                            @click="show = !show"
                            class="absolute inset-y-0 right-0 flex items-center pr-3.5 text-[#A39284] hover:text-[#2B1E19] focus:outline-none"
                            tabindex="-1"
                        >
                ...
                <div class="flex items-center justify-between pt-1">
                    <label for="remember_me" class="inline-flex cursor-pointer items-center">
                        <input
                            id="remember_me"
                            type="checkbox"
                            class="h-4 w-4 rounded-[5px] border-[#EADBCE] text-[#B38352] focus:ring-[#B38352]/30"
                            name="remember"
                        >
                        <span class="ms-2 text-sm text-[#736357] select-none">{{ __('Ghi nhớ tài khoản') }}</span>
                    </label>

                    @if (Route::has('password.request'))
                        <a class="text-sm font-semibold text-[#B38352] transition hover:text-[#8E6238] hover:underline" href="{{ route('password.request') }}">
                            {{ __('Quên mật khẩu?') }}
                        </a>
                    @endif
                </div>
            ...
            <!-- 3. Social Login Divider (kiểu vé giấy đục lỗ) -->
            <div class="relative my-8 flex items-center gap-3.5">
                ...
                <span class="whitespace-nowrap text-[11px] font-bold uppercase tracking-wider text-[#A39284]">{{ __('Hoặc tiếp tục với') }}</span>
                <div class="relative h-0 flex-1 border-t-[1.5px] border-dashed border-[#EADBCE]">
                    <span class="absolute -right-1 top-1/2 h-2.5 w-2.5 -translate-y-1/2 rounded-full bg-[#FAF5F1] ring-1 ring-[#EADBCE]"></span>
                </div>
            </div>

            <!-- 4. Social Login Buttons Grid (Google, Facebook, Twitter) -->
            <div class="relative grid grid-cols-3 gap-3">
                ...
                <a
                    ...
                    class="flex items-center justify-center rounded-2xl border border-[#EADBCE] bg-white py-3 px-3 transition duration-200 hover:-translate-y-0.5 hover:border-[#B38352] hover:bg-[#FAF5F1]/80 hover:shadow-sm"
                    title="{{ __('Đăng nhập với Facebook') }}"
                >
                ...
                <a
                    ...
                    class="flex items-center justify-center rounded-2xl border border-[#EADBCE] bg-white py-3 px-3 transition duration-200 hover:-translate-y-0.5 hover:border-[#B38352] hover:bg-[#FAF5F1]/80 hover:shadow-sm"
                    title="{{ __('Đăng nhập với X (Twitter)') }}"
                >
            ...
            <p class="relative mt-8 text-center text-[13.5px] text-[#736357]">
                ...
                <a href="{{ route('register') }}" class="font-bold text-[#B38352] transition hover:text-[#8E6238] hover:underline">
